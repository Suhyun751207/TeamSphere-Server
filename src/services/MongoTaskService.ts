...
async function getTasksByTeamUserIds(teamUserIds: number[]): Promise<MongoTaskDocument[]> {
  return await MongoTaskModel.find({ workspaceTeamUserId: { $in: teamUserIds } }).sort({ createdAt: -1 });
}
